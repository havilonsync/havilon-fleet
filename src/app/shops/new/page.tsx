...
export default function NewShopPage() {
  ...
  const [form, setForm] = useState({ name: '', address: '', phone: '', contactPerson: '', email: '', categories: '', notes: '' })
    ...
    try {
      const res = await fetch('/api/shops', {
        ...
        body: JSON.stringify({
          ...
          phone: form.phone.trim() || undefined, contactPerson: form.contactPerson.trim() || undefined,
      ...
      router.push('/shops?created=true'); router.refresh()
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="label">Shop Name *</label><input className="input" value={form.name} onChange={e => set('name', e.target.value)} required /></div>
        ...
        <div className="flex gap-3">
          <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Creating...' : 'Create Shop'}</button>
