    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          <thead>
            <tr>
              {['Unit #', 'VIN', 'License Plate', 'Make / Model / Year', 'Driver', 'Odometer', 'Repairs', 'Lifetime Spend', 'Status'].map(h => (
          ...
          <tbody>
            {vehicles.map(v => (
              <tr key={v.id} className="hover:bg-gray-50">
                ...
                <td className="table-cell text-sm">{v.licensePlate ?? <span className="text-gray-300">—</span>}</td>
                <td className="table-cell">{v.make} {v.model} {v.year}</td>
            ...
            {vehicles.length === 0 && (
              <tr><td colSpan={9} className="text-center py-12 text-sm text-gray-400">No vehicles yet — add your first vehicle to get started</td></tr>
            )}
          </tbody>
        </table>
      </div>
