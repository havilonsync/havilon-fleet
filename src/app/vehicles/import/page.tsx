...
interface ImportResult {
  imported: number
  skipped:  number
  errors:   { row: number; vin: string; reason: string }[]
}

const TEMPLATE_HEADERS = 'Unit #,VIN,Year,Make,Model,License Plate,Status'
const TEMPLATE_ROWS = [
  'VAN-001,1FTBW2CM5MKA00001,2021,Ford,Transit,ABC-1234,Active',
  'VAN-002,1FTBW2CM5MKA00002,2022,Ford,Transit,XYZ-5678,Active',
]
...
function downloadTemplate() {
  ...
  a.href     = url
  a.download = 'van-import-template.csv'
  a.click()
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <Link href="/vehicles" className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-2">
            <ArrowLeft size={14} /> Back to Vehicles
          </Link>
          <h1 className="text-xl font-semibold text-gray-900">Import Vans from Amazon Portal</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Export your fleet from the Amazon DSP portal as CSV, then upload it here.
          </p>
        </div>
      ...
      <div className="card p-5 bg-blue-50 border-blue-200">
        ...
        <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
          ...
          <li>Go to <strong>Fleet Management → Vehicles</strong></li>
          <li>Click <strong>Export</strong> or <strong>Download CSV</strong></li>
          <li>Upload the downloaded file below</li>
        </ol>
        <p className="text-xs text-blue-700 mt-3">
          Accepted column names: <span className="font-mono">Unit #, VIN, Year, Make, Model, License Plate, Status</span>
          &nbsp;(and common variations). Existing vehicles are skipped automatically.
        </p>
      </div>
        ...
        <div className="card p-6 space-y-4">
                  ...
                  <tbody>
                    {preview.rows.map((row, ri) => (
                      <tr key={ri} className="border-t border-gray-100">
                        {row.map((cell, ci) => (
                          <td key={ci} className="px-3 py-2 text-gray-700 whitespace-nowrap font-mono">{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
          ...
          {file && (
            <div className="flex gap-3">
              ...
              >
                {loading ? 'Importing…' : `Import Vehicles from "${file.name}"`}
              </button>
      ...
      {result && (
        <div className="card p-6 space-y-5">
                ...
                <table className="w-full text-xs">
                  <thead className="bg-red-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-medium text-red-700">VIN</th>
          ...
          <div className="flex gap-3">
            <Link href="/vehicles" className="btn-primary">View Vehicle Registry</Link>
